...
def move_up(index):
    if index > 0:  # Проверяем, что элемент не первый в списке
        schedule[index], schedule[index - 1] = schedule[index - 1], schedule[index]  # Меняем местами с предыдущим
        save_schedule()
    return redirect(url_for('schedule_page'))
...
def move_down(index):
    if index < len(schedule) - 1:  # Проверяем, что элемент не последний в списке
        schedule[index], schedule[index + 1] = schedule[index + 1], schedule[index]  # Меняем местами со следующим
        save_schedule()
    return redirect(url_for('schedule_page'))
