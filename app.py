    ...
    while True:
        ...
        for index, slot in enumerate(schedule):
            ...
            audio_file_path = os.path.join(
                app.config["UPLOAD_FOLDER"], slot["audio_file_path"]
            )

            # Воспроизведение в начале урока
            if now == lesson_start and not played_slots.get(index, {}).get(
                "start", False
            ):
                ...
                played_slots[index][
                    "start"
                ] = True  # Отмечаем, что воспроизведение на начало занятия выполнено

            # Воспроизведение в конце урока
            elif now == lesson_end and not played_slots.get(index, {}).get(
                "end", False
            ):
                play_audio_once(audio_file_path)
                if index not in played_slots:
                ...
                played_slots[index][
                    "end"
                ] = True  # Отмечаем, что воспроизведение на окончание занятия выполнено
...
def move_down(index):
    schedule = load_schedule()  # Загружаем расписание
    if (
        index >= 0 and index < len(schedule) - 1
    ):  # Убедимся, что индекс в допустимом диапазоне
        # Меняем элементы местами
        schedule[index], schedule[index + 1] = schedule[index + 1], schedule[index]
        save_schedule(schedule)  # Сохраняем изменённое расписание
